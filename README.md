...
# Module-4 : Gold Layer (Master Data Management)

## Architecture

```text
                    Healthcare_MDM - Gold Layer

                  Standardized Provider Records
                              │
                              ▼
             Healthcare_MDM.silver.provider_standardized
                              │
        ...
        ▼
                Data Quality Validation
                  ...
                  Golden Record Generation
                              │
                              ▼
                 ORIEO Payload Preparation
                              │
                              ▼
        Healthcare_MDM.gold.provider_golden
```
...
The Gold Layer is the business layer of the Healthcare Master Data Management (MDM) pipeline.

Its primary responsibility is to transform standardized provider records into trusted master records by applying business validation, data quality checks, matching logic, duplicate detection, survivorship rules, and Golden Record creation.

The final output is a business-ready provider dataset that can be consumed by downstream healthcare applications.
...
This table contains standardized provider records generated by the Silver Layer after parsing and cleaning the raw NPPES data.

The Gold Layer never reads raw JSON.

It only consumes standardized business data.

---

# Step-1 Business Rule Validation

Business Rules ensure that only valid healthcare providers continue through the MDM pipeline.

Implemented Rules

• ProviderID must exist.

• Provider Status must be Active.

• CountryCode must be US.

• TaxonomyCode must exist.
...
This provides complete auditability of provider validation.

---

# Step-2 Data Quality Validation

Business Rules and Data Quality are treated separately.
...
Identify providers having identical ProviderIDs.
...
Since only NPPES data is loaded,

all ProviderIDs are unique.

Future

When multiple source systems are integrated,

the same ProviderID can appear across
...
Exact Matching will automatically identify duplicate provider records.
...
Identify providers having similar business attributes.
...
Providers having identical contact information are marked as candidate matches.

These records are NOT automatically merged.

They are only flagged for review.

This avoids false-positive matching.
...
Duplicate Detection identifies multiple records belonging to the same provider.
...
All providers are UNIQUE because the project currently contains only one source system.

---

# Step-6 Survivorship

When duplicate providers exist,

the Survivorship Engine determines which provider record should become the enterprise master.

Implemented Rules

Prefer Active Provider

Prefer latest LastUpdated value

Assign Survivor Rank

Mark the selected record as SURVIVOR
...
Since every ProviderID is unique,

all records become SURVIVOR.

---

# Step-7 Golden Record Creation

Only records marked as SURVIVOR are retained.

These trusted records become the enterprise Golden Records.

Golden Records represent the single trusted version of every healthcare provider.

---

# Step-8 ORIEO Payload Generation

Golden Records are transformed into the format required by the downstream ORIEO application.
...
The generated payload is business-ready and can be consumed by downstream applications.

---

# Step-9 Gold Delta Table

The final Golden Records are stored in

Healthcare_MDM.gold.provider_golden

This Delta table becomes the trusted Provider Master.
...
can directly consume this table.

---

# Module Outcome

Successfully implemented the complete Gold Layer of the Healthcare Master Data Management pipeline.
...
✔ ORIEO Payload Generation

✔ Gold Delta Table Creation

The Healthcare MDM pipeline now follows the Medallion Architecture
